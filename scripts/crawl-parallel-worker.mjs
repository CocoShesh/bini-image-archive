...
function workerStateKey() { return `${R2_WORKER_PREFIX}/worker-${WORKER_ID}.json`; }
function workerShardKey() { return `${R2_WORKER_PREFIX}/worker-${WORKER_ID}-library.json`; }
...
function tagsFor(query) {
  ...
  return unique([inferCategory(query), ...TERMS.filter((term) => lower.includes(term.toLowerCase())).slice(0, 8)]);
}
...
function publicUrlFor(key) { return `${R2_PUBLIC_BASE_URL}/${key}`; }
async function uploadToR2(key, buffer) {
  if (!R2_ENABLED || !r2) throw new Error('R2 is not configured');
  await r2.send(new PutObjectCommand({
...
async function encodeForStorage(input) {
  return sharp(input, { failOn: 'none' }).rotate().webp({ quality: IMAGE_QUALITY, effort: 4 }).toBuffer();
}

async function loadBaseLibrary() {
  const library = await readR2Json(R2_LIBRARY_KEY, []);
  await fs.mkdir(DATA_DIR, { recursive: true });
  await writeJson(path.join(DATA_DIR, 'library-base.json'), library);
  return library;
}
async function loadWorkerShard() {
  const r2Shard = await readR2Json(workerShardKey(), []);
  const local = await readJson(path.join(LOCAL_WORKER_DIR, `worker-${WORKER_ID}-library.json`), []);
  const source = Array.isArray(r2Shard) && r2Shard.length ? r2Shard : local;
  await fs.mkdir(LOCAL_WORKER_DIR, { recursive: true });
  await writeJson(path.join(LOCAL_WORKER_DIR, `worker-${WORKER_ID}-library.json`), source);
  return source;
}
async function persistWorkerState(state, shard) {
  await fs.mkdir(LOCAL_WORKER_DIR, { recursive: true });
  await writeJson(path.join(LOCAL_WORKER_DIR, `worker-${WORKER_ID}.json`), state);
  await writeJson(path.join(LOCAL_WORKER_DIR, `worker-${WORKER_ID}-library.json`), shard);
  await writeR2Json(workerStateKey(), state);
  await writeR2Json(workerShardKey(), shard);
}
async function loadWorkerState() {
  const r2State = await readR2Json(workerStateKey(), null);
  if (r2State) return r2State;
  return readJson(path.join(LOCAL_WORKER_DIR, `worker-${WORKER_ID}.json`), null);
}
...
async function processImage(item, query, library, knownHashes) {
  try {
    ...
    let localPath = '';
    if (KEEP_LOCAL) {
      const dir = path.join(ROOT, 'public', 'library');
...
async function main() {
  if (!R2_ENABLED) throw new Error('Parallel mode requires Cloudflare R2 configuration.');
  if (!Number.isInteger(WORKER_ID) || WORKER_ID < 0 || WORKER_ID >= WORKER_COUNT) throw new Error('Invalid CRAWL_WORKER_ID');

  const queries = buildQueries();
  const signature = sha256(Buffer.from(queries.join('\n')));
  const totalQueries = queries.length;
  const baseIndex = Math.max(0, Math.min(PARALLEL_BASE_INDEX, totalQueries));
  const remaining = Math.max(0, totalQueries - baseIndex);
  const workerStart = baseIndex + Math.floor((remaining * WORKER_ID) / WORKER_COUNT);
  const workerEnd = baseIndex + Math.floor((remaining * (WORKER_ID + 1)) / WORKER_COUNT);

  const previous = await loadWorkerState();
  let startIndex = workerStart;
  if (previous?.signature === signature && previous?.startIndex === workerStart && previous?.endIndex === workerEnd && Number.isInteger(previous.nextQueryIndex)) {
    startIndex = Math.max(workerStart, Math.min(previous.nextQueryIndex, workerEnd));
  }

  const baseLibrary = await loadBaseLibrary();
  const workerLibrary = await loadWorkerShard();
  const knownHashes = new Set([...baseLibrary, ...workerLibrary].map((item) => item?.sha256).filter(Boolean));

  console.log('========================================');
  console.log(' BINI ARCHIVE V2 · 5-WAY PARALLEL CRAWLER');
  console.log('========================================');
  console.log(`Worker            : ${WORKER_ID + 1}/${WORKER_COUNT}`);
  console.log(`Total queries     : ${totalQueries}`);
  console.log(`Global base       : ${baseIndex}`);
  console.log(`Assigned range    : ${workerStart}..${Math.max(workerStart, workerEnd - 1)}`);
  console.log(`Resume            : ${startIndex}`);
  console.log(`Base library      : ${baseLibrary.length}`);
  console.log(`Worker additions  : ${workerLibrary.length}`);
  ...
  const stateBase = {
    ...
    status: startIndex >= workerEnd ? 'complete' : 'running',
    libraryTotal: workerLibrary.length,
    updatedAt: new Date().toISOString(),
  };
  await persistWorkerState(stateBase, workerLibrary);

  if (startIndex >= workerEnd) {
    console.log('WORKER ALREADY COMPLETE');
    return;
  }
  ...
  let addedCount = 0, duplicateCount = 0, failedCount = 0, discoveredCount = 0;

  try {
    for (let i = startIndex; i < workerEnd; i++) {
      const query = queries[i];
      const results = await crawlQuery(page, query);
      discoveredCount += results.length;
      for (const item of results) {
        const result = await processImage(item, query, workerLibrary, knownHashes);
      ...
      const state = {
        ...
        nextQueryIndex: i + 1,
        lastQuery: query,
        status: i + 1 >= workerEnd ? 'complete' : 'running',
        libraryTotal: workerLibrary.length,
        addedCount,
        duplicateCount,
        failedCount,
      ...
      await persistWorkerState(state, workerLibrary);
      console.log(`WORKER ${WORKER_ID + 1}/${WORKER_COUNT} · PROGRESS ${i + 1}/${workerEnd} · added ${addedCount} · dupes ${duplicateCount} · failed ${failedCount}`);
  ...
  console.log(`WORKER ${WORKER_ID + 1} DONE · next=${finalState?.nextQueryIndex ?? workerStart} · shard=${workerLibrary.length}`);
}
