...
async function uploadImage(item) {
  ...
  if (item.storageUrl && item.storageKey) {
    return {
  ...
  try {
    ...
    // Only use WebP when it is actually smaller.
    if (converted.length < source.length) {
      outputBuffer = converted;
      outputType = "image/webp";
    }
  } catch {
    // Preserve the original if optimization fails.
  }
  ...
  for (
    ...
    try {
      ...
      if (
        ...
        console.log(
          `[${i + 1}/${library.length}] uploaded ${item.sha256}`
        );
      } else if (
        ...
        console.log(
          `[${i + 1}/${library.length}] exists ${item.sha256}`
        );
      } else {
        skipped++;
      ...
      if (!checkpoint.completed.includes(item.sha256)) {
      ...
      console.error(
        `[${i + 1}/${library.length}] failed ${item.sha256}: ${error.message}`
      );
      ...
      // Do not advance past a failed item.
      checkpoint.nextIndex = i;
      checkpoint.updatedAt =
        new Date().toISOString();
      ...
      console.log(
        "Migration paused at failed item."
      );
